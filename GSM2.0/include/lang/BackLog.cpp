		...
		FDebug(false), FMaxFileSize(1024 * 1024 * 3), FPath(_path), FName(_name)
{
	...
	OpenCurrentFile();
	handle = CreateThread(NULL, 56 * 1024, (LPTHREAD_START_ROUTINE)BackLogCreateThread, (void*)this, CREATE_SUSPENDED, &pt);
	if (handle == NULL)  throw GetLastError();
	FQueue = new TMessageLog(handle);
}
...
void TBackLog::Execute()
{
	...
	while( (mlist = FQueue->GetList()) != NULL)
	{
		while(!mlist->empty())
		{
			std::string *data = mlist->front();
			try
			{
				WriteLog(data);
			}
